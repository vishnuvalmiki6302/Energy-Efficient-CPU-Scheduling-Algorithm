...
double adaptive_dvfs(int workload) {
    if (workload > 75) return MAX_FREQUENCY; // High workload, max frequency
    if (workload > 50) return MID_FREQUENCY; // Medium workload, mid frequency
    return MIN_FREQUENCY;                   // Low workload, minimum frequency
}

// Power Calculation
double calculate_power(double frequency, int is_idle) {
    return is_idle ? IDLE_POWER : BASE_POWER * (frequency / MAX_FREQUENCY);
}
